# WARNING !! run this script TWICE IN A ROW because it seems that set.seed(1) does not produce the same random values than seen in the paper (the problem is fixed for all other iterations of the script)

# this script produces all the plots seen in FIG 4-6

setwd("~/GitHub/ctbi_article/PartII.aggregation/step2_contamination.of.datasets/")
...
data.outlier <- data.frame(k.datasets=c(1,2,3),n.outliers=rep(NA,3),y.min=rep(NA,3),y.max=rep(NA,3),stringsAsFactors = F)
...
for(k.datasets in 1:3)
{
  ...
  data <- data.raw
  data[index.NA,2] <- NA
  add.min <- round(min0-abs(mean0-min0)*0.5,digits=2)
  add.max <- round(max0+abs(mean0-max0)*0.5,digits=2)
  if(k.datasets != 2)
  {
    data[index.min,2] <- add.min
    data[index.max,2] <- add.max

    data.outlier[k.datasets,'y.min'] <- add.min
    data.outlier[k.datasets,'y.max'] <- add.max
  }else
  {
    data[index.min,2] <- 1.6*max0
    data[index.max,2] <- 1.6*max0
    
    data.outlier[k.datasets,'y.min'] <- 1.6*max0
    data.outlier[k.datasets,'y.max'] <- 1.6*max0
  }


  ylim.min <- c(5,0,200)
  ylim.max <- c(50,350,1200)
  ...
  # save the data for the tsoutliers function (that does not handle real gaps)
  if(k.datasets==1)
  {
    tsoutliers.T <- data
    tsoutliers.T[index.blocks,2] <- NA
  }
  if(k.datasets==2)
  {
    tsoutliers.P <- data
    tsoutliers.P[index.blocks,2] <- NA
  }
  if(k.datasets==3)
  {
    tsoutliers.CH4 <- data
    tsoutliers.CH4[index.blocks,2] <- NA
  }

  # remove these values for ctbi
  data <- data[-index.blocks,]

  # calculate the true number of outliers
  data.outlier[k.datasets,'n.outliers'] <- sum(data[,2] == data.outlier[k.datasets,'y.min'] | data[,2] == data.outlier[k.datasets,'y.max'],na.rm=T)
  ...
  list.main.raw <- ctbi(data.raw,bin.side=bin.side,bin.period=bin.period,bin.center=bin.center,bin.FUN = bin.FUN,bin.max.f.NA = bin.max.f.NA,SCI.min = SCI.min,ylim=ylim,coeff.outlier=NA)
  ...
  if(k.datasets==2)
  {
    ...
    bin.center <- NULL
    bin.side <- as.Date('1907-01-01')
    SCI.min <- 0.6 # NA if no imputation
    ylim <- c(0,Inf)
    bin.max.f.NA <- 0.2
  }
  if(k.datasets==3)
...
# calculate the false positives and false negatives for this study
for(k.datasets in 1:3)
{
  if(k.datasets==1)
  {
  ...
  read.true.bin.accepted <- (data0[,'outliers'] == data.outlier[k.datasets,'y.min']) | (data0[,'outliers'] == data.outlier[k.datasets,'y.max'])
  read.true.bin.rejected <- ((data0[,2] == data.outlier[k.datasets,'y.min']) & (data0[,'index.bin'] < 0)) | ((data0[,2] == data.outlier[k.datasets,'y.max']) & (data0[,'index.bin'] < 0))

  read.false <- (data0[,'outliers'] != data.outlier[k.datasets,'y.min']) & (data0[,'outliers'] != data.outlier[k.datasets,'y.max'])

  n.true.outliers <- data.outlier[k.datasets,'n.outliers']

  N.false.positive <- sum(read.false,na.rm=T)
  N.false.negative <- n.true.outliers-sum(read.true.bin.accepted,na.rm=T)-sum(read.true.bin.rejected,na.rm=T)
...
for(k.datasets in 1:3)
{
  ...
  if(k.datasets==3)
  {
    data0 <- tsoutliers.CH4
  }

  add.shift <- min(data0[,2],na.rm=T)-0.1
  d.temp <- data0[,2]-add.shift # only strictly positive values

  value.flagged.BOXCOX <- d.temp[tsoutliers(as.numeric(d.temp),lambda='auto')$index]
  value.flagged.noBOXCOX <- d.temp[tsoutliers(as.numeric(d.temp),lambda=NULL)$index]

  read.false.BOXCOX <- (value.flagged.BOXCOX != (data.outlier[k.datasets,'y.min']-add.shift)) & (value.flagged.BOXCOX != (data.outlier[k.datasets,'y.max']-add.shift))
  read.false.noBOXCOX <- (value.flagged.noBOXCOX != (data.outlier[k.datasets,'y.min']-add.shift)) & (value.flagged.noBOXCOX != (data.outlier[k.datasets,'y.max']-add.shift))


  read.true.BOXCOX <- (value.flagged.BOXCOX == (data.outlier[k.datasets,'y.min']-add.shift)) | (value.flagged.BOXCOX == (data.outlier[k.datasets,'y.max']-add.shift))
  read.true.noBOXCOX <- (value.flagged.noBOXCOX == (data.outlier[k.datasets,'y.min']-add.shift)) | (value.flagged.noBOXCOX == (data.outlier[k.datasets,'y.max']-add.shift))

  n.true.outliers <- data.outlier[k.datasets,'n.outliers']

  N.false.positive.BOXCOX <- sum(read.false.BOXCOX,na.rm=T)
  N.false.positive.noBOXCOX <- sum(read.false.noBOXCOX,na.rm=T)

  N.false.negative.BOXCOX <- n.true.outliers-sum(read.true.BOXCOX,na.rm=T)
  N.false.negative.noBOXCOX <- n.true.outliers-sum(read.true.noBOXCOX,na.rm=T)
  ...
  print(paste0('false positive BOXCOX: ',N.false.positive.BOXCOX))
  print(paste0('false negative BOXCOX: ',N.false.negative.BOXCOX))

  print(paste0('false positive NO BOXCOX: ',N.false.positive.noBOXCOX))
  print(paste0('false negative NO BOXCOX: ',N.false.negative.noBOXCOX))
}
}
