setwd("~/Science/ctbi2/hess-2021-609-main_v1.0.0/PartII.aggregation/step2_contamination.of.datasets")
...
for(k.datasets in 1:3)
{
  ...
  stacked.cycles.index.raw <- list.main.raw$stacked.cycles.index
  stacked.cycles.index <- list.main$stacked.cycles.index
  ...
  if(k.datasets==1)
  {
    ...
    SCI.temperature <- stacked.cycles.index
    SCI.temperature.raw <- stacked.cycles.index.raw
  }
  if(k.datasets==2)
  {
    ...
    SCI.precipitation <- stacked.cycles.index
    SCI.precipitation.raw <- stacked.cycles.index.raw
  }
  if(k.datasets==3)
  {
    CH4.aggreg2 <- data1
    ...
    SCI.CH4 <- stacked.cycles.index
    SCI.CH4.raw <- stacked.cycles.index.raw
  }
}
