...
setwd("~/Science/ctbi12/PartII.aggregation/step2_contamination.of.datasets/")
...
for(k.datasets in 1:3)
{
  ...
  {
    data[index.min,2] <- 2*max0
    data[index.max,2] <- 2*max0

    data.outlier[k.datasets,'y.min'] <- 2*max0
    data.outlier[k.datasets,'y.max'] <- 2*max0
  }
