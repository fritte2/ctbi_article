...
ctbi.cycle <- function(data0,bin.size,outliers.checked)
{
  ...
  if(sum(!is.na(data0[,detrended])) != 0)
  {
    ...
    if(outliers.checked == FALSE)
    {
      plot(data0[,cycle.index],data0[,detrended],main='median',ylim=c(-10,10),pch=16)
      lines(FUN.cycle[,cycle.index],FUN.cycle[,detrended],col='cyan',lwd=2)
    }
    ##
    ...
    if(outliers.checked == FALSE)
    {
      hist(data0[,detrended]-data0[,cycle],10)
      abline(v=c(-5.62,5.48),col='red',lwd=2)
    }
    ##
  }else
  {
