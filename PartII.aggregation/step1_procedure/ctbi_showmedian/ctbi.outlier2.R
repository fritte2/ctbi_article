#' @title ctbi.outliers
#'
#' @description Flag the outliers with the Logbox method, which replaces the original constant 1.5 of the Boxplot rule with A*log(n)+B+C/n, with n being the sample size (n > 8, otherwise the MAD is used). Details are given in PAPER.
#'
#' @param y univariate data (numeric vector)
#' @param coeff.outlier One of coeff.outlier = \'auto\' (default value), coeff.outlier = \'gaussian\', coeff.outlier = c(A,B,C) or coeff.outlier = NA. If coeff.outlier = \'auto\', C = 50 and the coefficients A and B are calculated on m.star, a predictor of the kurtosis excess. If coeff.outlier = \'gaussian\', coeff.outlier = c(0.08,2,33), adapted to the Gaussian distribution. If coeff.outlier = NA, no outliers are flagged.
#'
#' @return A list that contains:
#' @return xy, a two columns data frame that contains the clean data (first column) and the outliers (second column).
#' @return summary.xy = c(A,B,C,m.star,lower.outlier.threshold,upper.outlier.threshold)
#' @examples
#' x <- 1
#' @export
...
ctbi.outlier <- function(y,coeff.outlier='auto')
{
  ...
  if(length(coeff.outlier) == 1)
  {
    ...
    }else
    {
      ...
      {
        log.user.coeff <- TRUE
        coeff.outlier <- c(0.08,2,33) # gaussian values
      }
    }
  }else
  {
  ...
  list.out <- list(xy=xy,summary.xy=summary.xy)
  return(list.out)
}
