...
#' @param coeff.outlier One of coeff.outlier = \'auto\' (default value), coeff.outlier = \'gaussian\', coeff.outlier = c(A,B,C) or coeff.outlier = NA. The original constant 1.5 of the Boxplot rule will be replaced with A*log(n)+B+C/n. If coeff.outlier = \'auto\', C = 50 and the coefficients A and B are calculated on m.star, a predictor of the kurtosis excess. If coeff.outlier = \'gaussian\', coeff.outlier = c(0.08,2,33), adapted to the Gaussian distribution. If coeff.outlier = NA, no outliers are flagged.
...
ctbi <- function(data.input,bin.side=NULL,bin.period,bin.center=NULL,bin.FUN = 'mean',bin.max.f.NA = 0.2,SCI.min = 0.6,coeff.outlier='auto',ylim=c(-Inf,+Inf))
{
  ...
  if(1)
  {
    ...
    # coeff.outliers
    char.coeff.outlier <- 'coeff.outlier is one of \'auto\' (default value), \'gaussian\', c(A,B,C) or NA. If coeff.outlier = \'auto\', C = 50 and the coefficients A and B are calculated on m.star, a predictor of the kurtosis excess. If coeff.outlier = \'gaussian\', coeff.outlier = c(0.08,2,33), adapted to the Gaussian distribution. If coeff.outlier = NA, no outliers are flagged.'
  ...
  data0 <- ctbi.long.term(data0,n.bin.min,seq.bin.side,seq.bin.center,outliers.checked=FALSE)
  ...
  data0[,outliers := .(fcoalesce(as.numeric(outliers),as.numeric(residuals)))] # merge both columns
  data0[!is.na(outliers),y:= NA_real_] # update y values
  data0[,residuals := NULL] # remove the column residuals
  summary.outlier <- list.out$summary.xy # contains A.coeff,B.coeff,C.coeff,m.star,n.residuals,lower.outlier.threshold and upper.outlier.threshold
  rm(list.out)

  # repeat steps 1,2,3 with the mean
  data0[,y := hidd.replace(y,N.min.NA=n.bin.min),by=index.bin]
  data0 <- ctbi.long.term(data0,n.bin.min,seq.bin.side,seq.bin.center,outliers.checked=T)
  ...
  ###
  plot(data0[,x],data0[,y],main='mean',ylim=c(0,25),pch=16)
  lines(data0[,x],data0[,cycle]+data0[,long.term],col='blue')
  ###
  ...
  list.main <- list(data0=data0,data1=data1,SCI=SCI,mean.cycle=mean.cycle,bin.size=bin.size,n.bin.min=n.bin.min,summary.outlier=summary.outlier)
