...
ctbi.long.term <- function(data0,n.bin.min,seq.bin.side,seq.bin.center,outliers.checked)
{
  ...
  ## manuscript only : plot
  y.int.side <- y.int[c(rep(c(TRUE,FALSE),(nrow(both.dt)-1)/2),TRUE)]
  y.int.center <- y.int[c(rep(c(FALSE,TRUE),(nrow(both.dt)-1)/2),FALSE)]
  data3 <- copy(data0)
  data3[!is.na(outliers), y := outliers]
  if(outliers.checked == FALSE)
  {
  plot(data0[,x],data0[,y],main='median',ylim=c(0,25),pch=16)
  }else
  {
  plot(data0[,x],data0[,y],main='mean',ylim=c(0,25),pch=16)
  }
  points(seq.bin.side,y.int.side,col='red',pch=16)
  points(seq.bin.center,y.int.center,col='blue',pch=16)
  ##
