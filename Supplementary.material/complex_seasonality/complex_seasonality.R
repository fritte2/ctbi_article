...
devtools::install_github("fritte2/ctbi") # if ctbi is not available on CRAN yet.
library(cosore)
library(data.table)
library(ctbi)
...
for(k.agg in 1:2) # perform 2 successive aggregations
{
  ...
  k.outliers <- k.outliers.successive[k.agg]
...
list.ctbi <- ctbi(data.agg[CSR_PORT == k.port,c('CSR_TIME','CSR_FLUX_CO2'),with=F],bin.FUN='median',SCI.min=SCI.min,bin.side=bin.side,bin.period=bin.period,k.outliers = Inf,bin.max.f.NA=bin.max.f.NA)
...
list.ctbi <- ctbi(data.low[,c('CSR_TIME','CSR_FLUX_CO2'),with=F],bin.FUN=bin.FUN,SCI.min=SCI.min,bin.side=bin.side,bin.period=bin.period,k.outliers = k.outliers,bin.max.f.NA=bin.max.f.NA)
...
list.ctbi <- ctbi(data.high[,c('CSR_TIME','CSR_FLUX_CO2'),with=F],bin.FUN=bin.FUN,SCI.min=SCI.min,bin.side=bin.side,bin.period=bin.period,k.outliers = k.outliers,bin.max.f.NA=bin.max.f.NA)
