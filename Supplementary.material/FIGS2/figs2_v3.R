setwd("~/Science/ctbi12/Supplementary.material/FIGS2/")
set.seed(1)
...
for(k.period in 1:length(bin.period.all))
{
  ...
  for(k.bins in 1:length(number.of.bins))
  {
    ...
    for(k.replica in 1:number.of.replica[k.bins])
    {
      ...
      list.main <- ctbi(data0,bin.side=bin.side,bin.period=bin.period,bin.center=bin.center,bin.FUN = bin.FUN,bin.max.f.NA = bin.max.f.NA,SCI.min = SCI.min,coeff.outlier=coeff.outlier)
      data1 <- list.main$data1
      
      stacked.cycles.index <- list.main$summary.bin['SCI']+(1/sum(data1[,'index.bin'] > 0))
...
write.table(biased.stacked.cycles.index,'biased.stacked.cycles.index.R2_v2.csv',row.names = F,sep=',')
}else
{
